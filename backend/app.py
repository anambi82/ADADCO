...
latest_analysis = {"results": None, "timestamp": None, "filename": None}
...
def calculate_precision_recall_f1(y_true, y_pred, num_classes):
    ...
    weights = (
        np.array(support_per_class) / total_support
        if total_support > 0
        else np.ones(num_classes) / num_classes
    )
...
def evaluate():
    ...
    try:
        ...
        if input_data.shape[0] != len(true_labels):
            return jsonify({"error": "Number of samples and labels must match"}), 400
        ...
        metrics = calculate_precision_recall_f1(
            true_labels, predictions_np, len(classes)
        )
...
def get_anomaly_timeline():
    ...
    for i in range(num_windows):
        ...
        window_anomalies = [
            a for a in anomalies if start_idx <= a["sample_index"] < end_idx
        ]
...
def analyze_file():
    ...
    try:
        ...
        if "file" not in request.files:
            return (
                jsonify({"error": "No file provided. Please upload a CSV file."}),
                400,
            )
        ...
        required_features = scaler_stats["columns"]
        missing_features = []
        available_features = []
        ...
        logger.info(
            f"Extracted {len(available_features)} features from CSV with {len(df.columns)} total columns"
        )
        ...
        attack_types = dict(
            sorted(attack_types.items(), key=lambda x: x[1]["count"], reverse=True)
        )

        # Find high-confidence anomalies (for detailed view)
        anomaly_details = []
        for i, (pred_idx, confidence) in enumerate(
            zip(all_predictions, all_confidences)
        ):
            class_name = classes[pred_idx]
            if class_name != "BENIGN":
        ...
        top_anomalies = sorted(
            anomaly_details, key=lambda x: x["confidence"], reverse=True
        )[:100]
        ...
        logger.info(
            f"Analysis complete: {anomaly_count}/{total_samples} anomalies detected ({anomaly_rate:.2f}%)"
        )
        ...
        response_data = {
            ...
            "summary": {
                "most_common_attack": (
                    max(attack_types.items(), key=lambda x: x[1]["count"])[0]
                    if attack_types
                    else None
                ),
                ...
                "severity": (
                    "HIGH"
                    if anomaly_rate > 50
                    else "MEDIUM" if anomaly_rate > 20 else "LOW"
                ),
            },
        }
